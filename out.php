<?php 
    ...
    $logs = mysqli_query($connection, "select `logs`.id, `logs`.`desc`, `logs`.`value`, categories.`name` from `logs` JOIN categories on categories.id = `logs`.category_id where categories.type='out' and `logs`.deleted_at is NULL ORDER BY `logs`.id ASC");

    // sum uang keluar
    $query_sum = mysqli_query($connection, "select SUM(`value`) as sum_in from `logs` JOIN categories on categories.id = `logs`.category_id where categories.type='out' and `logs`.deleted_at is NULL");
            ...
            <div class="row">
                ...
                <div class="column25">
                    ...
                    <form action="action.php" method="post">
                        ...
                        <input  type="submit" name="add_out" style="background-color: #f0ad4e;"></input>
                        </div>
                    </form>
                </div>
                <div class="column75">
                    <div class="card" style="background-color: #5bc0de;">
                        Catatan Keluar : Rp <?php echo number_format($sum[0]);?>
                    </div>
                    <table id="table_in" class="display" style="width:100%">
                        ...
                        <tbody>
                            <?php while($row = mysqli_fetch_array($logs)){
                                ...
                                echo '<td>
                                    <a href="log_out_form.php?get_log='.$row['id'].'" class="btn-action"><i class="fas fa-edit"></i> Edit</a>
                                    <a href="action.php?delete_log='.$row['id'].'" class="btn-action"><i class="fas fa-trash"></i> Hapus</a></td>';
                                echo '</tr>';
                            } ?>
                        </tbody>
                    </table>
